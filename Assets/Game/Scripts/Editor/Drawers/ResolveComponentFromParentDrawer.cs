...
[CustomPropertyDrawer(typeof(ResolveComponentFromParentAttribute))]
public class ResolveComponentFromParentDrawer : PropertyDrawer
{
    private bool hierarchyChanged = false;

    public ResolveComponentFromParentDrawer()
    {
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        {
            this.ResolveComponentFromParentLogic(property);
    ...
    private void ResolveComponentFromParentLogic(SerializedProperty property)
    {
        ...
        {
            Debug.LogError($"`{nameof(ResolveComponentFromParentAttribute)}` is not valid " +
                $"for field `{this.fieldInfo.FieldType}` in Object `{targetObject.name}`!");
            return;
        }

        if (this.attribute is not ResolveComponentFromParentAttribute attributeInstance)
        {
            Debug.LogError($"`{nameof(ResolveComponentFromParentAttribute)}` cannot be found " +
