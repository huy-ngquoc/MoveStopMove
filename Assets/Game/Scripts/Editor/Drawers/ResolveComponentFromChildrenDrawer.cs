...
[CustomPropertyDrawer(typeof(ResolveComponentFromChildrenAttribute))]
public class ResolveComponentFromChildrenDrawer : PropertyDrawer
{
    private bool hierarchyChanged = false;

    public ResolveComponentFromChildrenDrawer()
    {
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        {
            this.ResolveComponentFromChildrenLogic(property);
    ...
    private void ResolveComponentFromChildrenLogic(SerializedProperty property)
    {
        ...
        {
            Debug.LogError($"`{nameof(ResolveComponentFromChildrenAttribute)}` is not valid " +
                $"for field `{this.fieldInfo.FieldType}` in Object `{targetObject.name}`!");
            return;
        }

        if (this.attribute is not ResolveComponentFromChildrenAttribute attributeInstance)
        {
            Debug.LogError($"`{nameof(ResolveComponentFromChildrenAttribute)}` cannot be found " +
