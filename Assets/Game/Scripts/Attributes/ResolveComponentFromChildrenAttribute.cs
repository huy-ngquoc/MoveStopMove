...
public sealed class ResolveComponentFromChildrenAttribute : PropertyAttribute
{
    /// <summary>
    /// Initializes a new instance of the <see cref="ResolveComponentFromChildrenAttribute"/> class
    /// to automatically assign a component from the GameObject or any of its children, excluding inactive ones by default.
    /// </summary>
    /// <remarks>
    /// The field must be serialized by Unity for this attribute to work.
    /// </remarks>
    public ResolveComponentFromChildrenAttribute()
    {
    }
    ...
    public ResolveComponentFromChildrenAttribute(string targetChildName)
    {
    ...
    public ResolveComponentFromChildrenAttribute(bool isIncludingInactive)
    {
    ...
    /// <param name="targetChildName"> The name of the GameObject or its children to search within. Leave blank to search all.</param>
    /// <param name="isIncludingInactive"> If `true`, includes inactive GameObjects in the search. If `false`, ignores them.</param>
    public ResolveComponentFromChildrenAttribute(string targetChildName, bool isIncludingInactive)
    {
        this.TargetChildName = targetChildName;
        this.IsIncludingInactive = isIncludingInactive;
    }

    public string TargetChildName { get; } = string.Empty;
